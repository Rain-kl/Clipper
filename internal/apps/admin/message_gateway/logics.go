...
func createChannel(ctx context.Context, req CreateChannelRequest) (ChannelDTO, error) {
	...
	cipher, err := appgw.EncryptCredentials(creds)
	if err != nil {
		return ChannelDTO{}, err
	}
	enabled := true
...
func updateChannel(ctx context.Context, id uint64, req UpdateChannelRequest) (ChannelDTO, error) {
	...
	row.Extra = appgw.EncodeExtra(extra)
...
func listChannels(ctx context.Context) ([]ChannelDTO, error) {
	...
	for i := range rows {
		creds, err := appgw.DecryptCredentials(rows[i].Credentials)
		if err != nil {
			creds = map[string]string{}
		}
		out = append(out, toDTO(&rows[i], creds, appgw.ParseExtra(rows[i].Extra)))
	}
	return out, nil
}
